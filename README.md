...
* ### SVM  
1. 函数间隔与几何间隔    
    * $|w^Tx + b&nbsp;|$表示点x到超平面的距离。通过观察$w^Tx + b $和y是否同号，判断分类是否正确,定义函数间隔$\gamma^{'}$为：$$\gamma^{'} = y(w^Tx + b)$$  
    可以看到，它就是感知机模型里面的误分类点到超平面距离的分子。对于训练集中m个样本点对应的m个函数间隔的最小值，就是整个训练集的函数间隔。函数间隔并不能正常反应点到超平面的距离，当分子成比例的增长时，分母也是成倍增长。为了统一度量，对法向量$w$加上约束条件，这样就得到了几何间隔$\gamma$,定义为：$$\gamma = \frac{y(w^Tx + b)}{||w||_2} = &nbsp;\frac{\gamma^{'}}{||w||_2}$$  
...
通过拉格朗日函数将我们的优化目标转化为无约束的优化函数，最大熵模型原理小结</a>中讲到了目标函数的优化方法一样。具体的，优化函数转化为：$$L(w,b,\alpha) = \frac{1}{2}||w||_2^2 - \sum\limits_{i=1}^{m}\alpha_i[y_i(w^Tx_i + b) - 1] \; 满足\alpha_i \geq 0$$  
